 ...
 */
export function isControlPlanePath(normalized: string): boolean {
...
function validatePath(p: string): string | null {
  ...
  if (p.trim() !== p) return `leading/trailing whitespace: ${JSON.stringify(p)}`;
  ...
  if (p.includes("\\")) return `backslash separator (POSIX paths only): ${JSON.stringify(p)}`;
  if (p.startsWith("/")) return `absolute path: ${JSON.stringify(p)}`;
  if (/^[A-Za-z]:/.test(p)) return `drive-letter path: ${JSON.stringify(p)}`;
  if (p.endsWith("/")) return `directory (trailing slash): ${JSON.stringify(p)}`;
  if (GLOB_CHARS.test(p)) return `glob metacharacter: ${JSON.stringify(p)}`;
  if (ANGLE_CHARS.test(p)) return `angle bracket (template placeholder?): ${JSON.stringify(p)}`;
  const segments = p.split("/");
  if (segments.some((s) => s.length === 0)) return `empty segment (//): ${JSON.stringify(p)}`;
  if (segments.some((s) => s === "." || s === "..")) {
    return `traversal segment (. or ..): ${JSON.stringify(p)}`;
  }
  if (isControlPlanePath(p)) return `control-plane path is always forbidden: ${JSON.stringify(p)}`;
  return null;
}
 ...
 */
export function parseAllowlist(raw: readonly string[]): AllowlistParseResult {
  const errors: string[] = [];
 ...
 */
export function parseFilesBlockFromCard(cardText: string): AllowlistParseResult {
  ...
  if (current !== null) return { ok: false, errors: ["unclosed ```files block"] };
  if (blocks.length === 0) {
...
export function canonicalSetsEqual(a: readonly string[], b: readonly string[]): boolean {
