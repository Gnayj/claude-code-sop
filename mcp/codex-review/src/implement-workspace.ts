...
import { type DiffBudget, type PatchEntry, buildGitPatch } from "./diff.js";
...
export function resolveControlDir(repoRoot: string, segments: readonly string[]): string {
  ...
  for (const segment of [".codex-review", ...segments]) {
    ...
    if (st.isSymbolicLink()) {
      throw new ControlRootViolation(
        `control-state path component is a symlink (refusing to follow): ${current} -> ${readlinkSync(current)}`,
      );
    }
    if (!st.isDirectory()) {
...
export function resourceRoot(repoRoot: string): string {
...
export function dispatchResourcePaths(repoRoot: string, artifactId: string): DispatchResources {
...
export function allocateDispatchResources(repoRoot: string, artifactId: string): DispatchResources {
...
export function discardDispatchResources(repoRoot: string, artifactId: string): void {
...
export function ensureFlockSupport(repoRoot: string): void {
  if (flockProbed) return;
...
function assertNoFollowComponents(root: string, relPath: string): void {
...
export class BlobStore {
  ...
  readVerified(sha: string): Buffer {
    ...
    if (actual !== sha) {
      throw new Error(
        `sealed blob hash mismatch (tamper tripwire): expected ${sha}, got ${actual}`,
      );
    }
    return bytes;
  }
 ...
 * reopened.
 */
export function readFileToStore(root: string, relPath: string, store: BlobStore): SpooledRead {
  assertNoFollowComponents(root, relPath);
  const full = join(root, relPath);
  const pre = lstatSync(full);
  if (pre.isSymbolicLink()) throw new UnsafePathError(`symlink final component: ${full}`);
  ...
  try {
    ...
    if (!st.isFile()) throw new UnsafePathError(`not a regular file after open: ${full}`);
...
// ---------- snapshot (design §4.2 step 1: typed kinds + topology pass) ----------

export type NonRegularKind = "symlink" | "dir" | "gitlink" | "other";

export type EntryState =
  | { state: "present"; kind: "file"; mode: "100644" | "100755"; sha: string; size: number }
  | { state: "present"; kind: NonRegularKind }
  | { state: "absent" };
...
export function listTrackedWithStage(repoRoot: string): TrackedEntry[] {
...
function statPathToStore(
  ...
  if (gitlinkPaths.has(path)) return { state: "present", kind: "gitlink" };
  let st;
  try {
    ...
    if (err instanceof UnsafePathError) return { state: "present", kind: "other" };
    throw err;
  }
  if (st.isFile()) {
  ...
  if (st.isSymbolicLink()) return { state: "present", kind: "symlink" };
  if (st.isDirectory()) return { state: "present", kind: "dir" }; // opaque root (r7), not absent
...
export function buildSnapshot(
  ...
  const inventory = new Map<string, EntryState>();
  ...
  for (const [path, entry] of inventory) {
    if (entry.state === "present" && entry.kind !== "file") opaqueRoots.add(path);
  }
  ...
  for (const entry of allowlist) {
    for (const root of opaqueRoots) {
      if (isEqualOrBelow(entry, root)) {
        const kind =
          inventory.get(root)?.state === "present"
            ? (inventory.get(root) as { kind: string }).kind
            : "opaque";
        rejections.push({
          reason:
            entry === root
...
// ---------- scratch materialization (design §4.2 step 2) ----------
...
export function materializeScratch(snapshot: Snapshot, scratchRoot: string): ScratchWorkspace {
  ...
  const sgit = (args: string[]) =>
    execFileSync("git", args, { cwd: root, env: scratchEnv, maxBuffer: 64 * 1024 * 1024 });
  sgit(["init", "-q"]);
...
export function sealCapture(
  scratchRoot: string,
  store: BlobStore,
  opaqueRoots: ReadonlySet<string> = new Set(),
): Snapshot {
  const inventory = new Map<string, EntryState>();
...
// ---------- validation (design §4.2 step 3: Q9 end-state deltas + both-sides text gate +
// opaque-root baseline semantics) ----------
...
function isNulFree(bytes: Buffer): boolean {
...
export function validateCapture(
  ...
  const deltas: Delta[] = [];
  const paths = new Set<string>([...snapshot.inventory.keys(), ...capture.inventory.keys()]);
  for (const path of [...paths].sort()) {
    ...
    const key = (e: EntryState): string =>
      e.state === "present" && e.kind === "file"
        ? `f:${e.mode}:${e.sha}`
        : e.state === "present"
          ? `x:${e.kind}`
          : "absent";
    ...
    const op: Delta["op"] =
      before.state === "absent" ? "create" : after.state === "absent" ? "delete" : "modify";
    deltas.push({ path, op });
    if (!allowed.has(path)) {
    ...
    if (after.state === "present" && after.kind !== "file") {
      violations.push(
        `allowlisted postimage kind is not a regular file (${after.kind}) — typed violation: ${path}`,
      );
      continue;
    }
    ...
    for (const [label, side, inv] of [
      ...
      if (side.state !== "present") continue;
      if (side.kind !== "file") {
        violations.push(
          `allowlisted ${label} kind is not a regular file (${side.kind}) — typed violation: ${path}`,
        );
        continue;
      }
...
// ---------- patch generation (design §4.2 step 4 / §4.2.D / Q18 — GITLESS) ----------
...
export function generatePatch(
  ...
  const fileSide = (
    inv: Snapshot,
    path: string,
  ): { present: boolean; mode?: "100644" | "100755"; bytes?: Buffer } => {
    const e = inv.inventory.get(path);
    if (!e || e.state !== "present" || e.kind !== "file") return { present: false };
    return { present: true, mode: e.mode, bytes: inv.store.readVerified(e.sha) };
  };
  const entries: PatchEntry[] = [...deltas]
  ...
  const filesChanged: FileChangeFact[] = entries.map((e) => {
    const before = snapshot.inventory.get(e.path);
    const after = capture.inventory.get(e.path);
    const f = (entry: EntryState | undefined) =>
      entry && entry.state === "present" && entry.kind === "file" ? entry : null;
    const counts = built.perFile.get(e.path) ?? { added: 0, removed: 0, usedFallback: false };
    const op: Delta["op"] = !e.before.present ? "create" : !e.after.present ? "delete" : "modify";
    return {
      path: e.path,
      op,
...
// ---------- artifact publication (design §4.2.E) ----------
...
function writeDurable(finalPath: string, bytes: Buffer): void {
...
export function validateDispatchKey(key: string): string | null {
  const bytes = Buffer.byteLength(key, "utf8");
  if (bytes < 1 || bytes > 128) return `dispatch_key must be 1-128 UTF-8 bytes (got ${bytes})`;
...
export function isEpochAlive(pid: number, startToken?: string | null): boolean {
  if (startToken == null) return isPidAlive(pid);
  const current = readProcStartToken(pid);
  if (current == null) return false; // no such pid → dead
...
export function encodeDesignIdForFilename(designId: string): string {
  let out = "";
...
export class ImplementStore {
  ...
  private statePath(designId: string): string {
  ...
  lock(
    ...
    return acquireFlock(
      join(this.stateDir(), `${encodeDesignIdForFilename(designId)}.implement.lock`),
      deadline,
      signal,
    );
  }
  ...
  private readStateFile(path: string, expectDesignId?: string): ImplementSessionState | null {
    let text: string;
    try {
    ...
    if (expectDesignId !== undefined && parsed.design_id !== expectDesignId) {
      throw new Error(
        `state file ${path} belongs to design_id ${JSON.stringify(parsed.design_id)}, not ${JSON.stringify(expectDesignId)}`,
      );
    }
  ...
  private readAllStates(): { states: ImplementSessionState[]; complete: boolean } {
    const dir = this.stateDir();
    const states: ImplementSessionState[] = [];
    let complete = true;
  ...
  private reapStateTmpOrphans(): void {
    const dir = this.stateDir();
    for (const name of readdirSync(dir)) {
      if (STATE_TMP_RE.test(name)) rmSync(join(dir, name), { force: true });
    }
  }
  ...
  async recoverAndGc(
    ...
    deadline: AcquisitionDeadline,
    signal?: AbortSignal,
  ): Promise<ImplementSessionState | null> {
    ...
    if (state) {
      ...
      for (const record of state.dispatches) {
        ...
        if (!terminal && !isEpochAlive(record.epoch_pid, record.epoch_start_token)) {
          ...
          if (record.round > state.rounds) state.rounds = record.round;
          dirty = true;
        }
      }
  ...
  private gcResidue(allStates: readonly ImplementSessionState[], complete: boolean): void {
    const byArtifact = new Map<string, DispatchRecord>();
    ...
    for (const name of readdirSync(root)) {
      if (!ARTIFACT_ID_RE.test(name)) {
        ...
        if (name.startsWith(".flock-probe.")) rmSync(join(root, name), { force: true });
        continue;
      }
      ...
      if (reap) rmSync(join(root, name), { recursive: true, force: true });
    }
  }
  ...
  private async gcPublishedArtifacts(
    allStates: readonly ImplementSessionState[],
    complete: boolean,
    deadline: AcquisitionDeadline,
    signal?: AbortSignal,
  ): Promise<void> {
...
// ---------- isolated writer environment (design §4.2.C / Q11 + §4.2.F / Q19) ----------
...
export function buildWriterEnvironment(homeDir: string, model?: string): WriterEnvironment {
  ...
  configLines.push(
    ...
    "# the scratch subtree (sealed stores/staging are unreachable by path grant AND tmp channel).",
