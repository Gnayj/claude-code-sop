...
export class FlockUnavailableError extends Error {
    constructor(detail) {
        super(`util-linux \`flock\` is unavailable or unusable on this system/filesystem: ${detail}. ` +
...
export function acquisitionDeadline(totalMs) {
 ...
 */
export function probeFlockSupport(dir) {
    ...
    try {
        ...
        if (r.error)
            throw new FlockUnavailableError(`spawn failed: ${r.error.message}`);
        if (r.status !== 0) {
            throw new FlockUnavailableError(`probe lock attempt exited ${r.status ?? `signal ${r.signal}`}`);
        }
    }
    finally {
        closeSync(fd);
 ...
 */
export async function acquireFlock(lockPath, deadline, signal) {
    if (signal?.aborted)
        throw new LockCancelledError(lockPath);
    ...
    try {
        ...
        const acquired = await new Promise((resolvePromise, rejectPromise) => {
            ...
            child.once("error", (err) => {
                ...
                rejectPromise(new FlockUnavailableError(err.message));
            });
            child.once("exit", (code) => {
                signal?.removeEventListener("abort", onAbort);
                if (signal?.aborted)
                    rejectPromise(new LockCancelledError(lockPath));
                else
