...
export async function runReviewFlow(deps, input) {
    ...
    try {
        ...
        if (existingState && !existingState.archived) {
            ...
            else if (existingState.context_usage_pct >= cb.context_force_new_thread_pct) {
                didRebuildThisCall = true;
            ...
            if (didRebuildThisCall && rebuildReason) {
                ...
                coldStartPreface = renderColdStartPreface(recent, existingState.context_usage_pct, rebuildReason);
            }
        }
        const drift = planDrift(existingState, input.designDocPaths, (p) => resolveProjectPath(config, configBaseDir, p));
        ...
        if (shouldResume && existingState) {
            prior = {
                ...
                context_usage_source: existingState.provider_kind === "codex" ? "native" : "estimated",
        ...
        // are always fresh.
        const manualVerdictSha = provider.kind === "manual"
        ...
        if (existingState && !existingState.archived) {
            ...
            state = threadManager.newState(input.designId, runResult.providerSessionId, provider.kind);
        ...
        const tokens = runResult.usage.total ??
        ...
        else if (!didRebuildThisCall &&
            state.context_usage_pct >= cb.context_warn_pct &&
            state.context_usage_pct < cb.context_force_new_thread_pct) {
            warnings.push(`context_usage_pct=${state.context_usage_pct.toFixed(2)} crossed warn=${cb.context_warn_pct}; ` +
                `next call may rebuild thread.`);
        }
        else if (!didRebuildThisCall && state.context_usage_pct >= cb.context_force_new_thread_pct) {
            warnings.push(`context_usage_pct=${state.context_usage_pct.toFixed(2)} >= force-rebuild=${cb.context_force_new_thread_pct}; ` +
...
function currentRoundFor(state, stage) {
    if (!state)
        return 0;
    if (stage === "design")
        return state.rounds.design_review;
    if (stage === "code")
        return state.rounds.code_review;
...
//# sourceMappingURL=run-review-flow.js.map