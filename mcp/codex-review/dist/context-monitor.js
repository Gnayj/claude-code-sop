 ...
 */
export function decideContext(config, usagePct, alreadyRebuiltThisCall = false) {
    ...
    if (alreadyRebuiltThisCall) {
        return {
            severity: "exhausted",
            usagePct,
            message: `context_usage_pct=${usagePct.toFixed(2)} remains >= warn=${cb.context_warn_pct} ` +
    ...
    return {
        ...
        usagePct,
        message: `context_usage_pct=${usagePct.toFixed(2)} >= force-rebuild=${cb.context_force_new_thread_pct}; ` +
