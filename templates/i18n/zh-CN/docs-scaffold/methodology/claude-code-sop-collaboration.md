...
3.A **主持模式（仅 `claude+codex`）："Claude 主持 + codex 写手" —— 不切 CLI。**
   ...
   Claude session 内跑完：driver 出 design、写 implement 卡（仍然必出 —— 它就是派工契约，§4.1；
   其 ```files 块是机器解析的写入白名单），然后经桥工具 **`codex_implement`**（config
   `[implement] enabled=true`，`/sop-init` 仅在此流程下开启；默认关）向 codex 写手派发有界工单。
   该工具**从不写你的仓库**：codex 在隔离 scratch 里干活；server 校验结果（任何越白名单的终态
   delta ⇒ 整单拒绝、不出工件）并在 `.codex-review/dispatches/` 下返回 **patch 工件**。driver
